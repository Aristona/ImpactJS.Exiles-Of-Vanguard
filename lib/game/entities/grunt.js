...
.defines(function() {
	EntityGrunt = ig.Entity.extend({
		...
		//the defaults
		name: 'Boss Grunt',		/* Let's call him the Boss*/
		size: {
			x:16,
			y:40
		},  /* the default size */
		...
		health: 7500, 		/* And make him healthier than every other enemy*/
		maxHealth: 7500,
		zIndex:1,			/* He isn't more important to show than the player so if they occupy the same space show him below the player*/
		speed:10, 			/* The default speed is higher than an enemy*/
		animSheet: new ig.AnimationSheet('media/graphics/entities/boss_grunt.png',32,48),	/* where to find the animation sheet */
		...
		update: function(){
			...
			ig.game.font.interface.draw("BOSS HP: " + this.health, 140, 430);
		...
		receiveDamage: function(amount,from){
			...
			{
				// XP versin
				ig.game.experience += 50;

				//Drop versin

				for(i = 0; i < 4; i++)
				{
					ig.game.spawnEntity(EntityGold, (Math.floor((Math.random()*60) + this.pos.x)), (Math.floor((Math.random()*60) + this.pos.y)));
				}
